...
namespace Microsoft.AspNet.WebHooks
{
    ...
    public static class SlackCommand
    {
        ...
        public static KeyValuePair<string, string> ParseActionWithValue(string text)
        {
            ...
            var values = text.Split(LwsSeparator, 2, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
            if (values.Length == 0)
            {
                return new KeyValuePair<string, string>(string.Empty, string.Empty);
            }
        ...
        public static KeyValuePair<string, NameValueCollection> ParseActionWithParameters(string text)
        {
            ...
            var keyValuePairs = encodedSeparators.SplitAndTrim(';');
            foreach (var p in keyValuePairs)
            {
                var parameter = p.Split(EqualSeparator, 2, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
        ...
        internal static void ValidateParameterName(string name)
        {
            ...
            {
                var message = string.Format(CultureInfo.CurrentCulture, SlackReceiverResources.Receiver_CommandNameQuotedString, name);
                throw new ArgumentException(message);
            }
            if (name.IndexOf("\\\0", StringComparison.Ordinal) > -1)
            {
                name = name.Replace("\\\0", ";");
                var message = string.Format(CultureInfo.CurrentCulture, SlackReceiverResources.Receiver_CommandNameInvalid, name);
                throw new ArgumentException(message);
            }
        }
        ...
        internal static string EncodeNonSeparatorCharacters(string text)
        {
            ...
            while (true)
            {
                ...
                if (text[bytesConsumed] == '\'' || text[bytesConsumed] == '"')
                {
                    ...
                    {
                        var message = string.Format(CultureInfo.CurrentCulture, SlackReceiverResources.Receiver_CommandUnmatchedQuote, quote, quoteOffset);
                        throw new ArgumentException(message);
                    }
